...
async function runPgMigrations(q) {
  ...
  if (!columns.length && !added.length) logger.info('migration: schema already current');
  return { columns, foreignKeys: added, cleaned };
}

module.exports = { runPgMigrations, FOREIGN_KEYS, DENORM_COMPANY_ID };
