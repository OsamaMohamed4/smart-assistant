...
function summarizeReport(rows) {
  ...
  return {
    ...
    // Completion rate — "معدل إكمال المكالمة": of the calls that were actually
    // answered, how many ran the scenario to its close (vs ended early or
    // transferred). Denominator is `answered`, not `dialled`, so no-answers and
    // bad numbers — which never had a chance to complete — don't drag it down.
    completionRate: answered ? Math.round((outcomes[OUTCOME.COMPLETED] / answered) * 100) : 0,
