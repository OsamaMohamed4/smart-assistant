...
async function requireAuth(req, res, next) {
  try {
    ...
    // Narrow the DB tenant context (Task #2 RLS): superadmins keep the
    // system-wide bypass because they legitimately aggregate across tenants;
    // a client is pinned to its own company so Postgres enforces isolation
    // even if a query forgets its company_id filter.
    if (req.dbContext) {
      if (ctx.user.role === 'superadmin') {
        req.dbContext.bypass = true;
        req.dbContext.companyId = null;
      } else if (ctx.user.companyId) {
        req.dbContext.bypass = false;
        req.dbContext.companyId = ctx.user.companyId;
      }
    }
    next();
  } catch (e) { next(e); }
