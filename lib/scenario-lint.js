...
// Markdown that TTS would read literally: bold/italic markers, headings,
// bullet/numbered lists, table pipes, code backticks.
...
function lintScenario(text) {
  ...
  // 1) Diacritics — the #1 cause of garbled company-name pronunciation.
  // Threshold of 2 avoids flagging the odd legitimate kasra (e.g. "استدعِ");
  // it's pervasive tashkeel like "وَكَنْ" (fatha + sukoon) we care about.
  const dia = text.match(DIACRITICS);
  if (dia && dia.length >= 2) {
    warnings.push({
      code: 'diacritics', level: 'error', count: dia.length,
      message: 'يوجد تشكيل (حركات) في النص — يسبب نطقاً خاطئاً في الصوت (مثلاً "وَكَنْ" تُنطق غلط). اكتب الكلمات بدون تشكيل، خصوصاً اسم الشركة.',
    });
  }
  ...
  // 3) Markdown formatting — read literally on voice.
  if (MARKDOWN.test(text)) {
    warnings.push({
      code: 'markdown', level: 'warn',
      message: 'يوجد تنسيق ماركداون (**، #، قوائم، جداول) — يُقرأ حرفياً في الصوت. أزل التنسيق واكتب جُملاً عادية.',
    });
  }

  // 4) English words (outside {{placeholders}}) — odd pronunciation.
  ...
  // 5) No end-call instruction — agent may never hang up.
