...
// Default is 3-large: measured on a 500-chunk Saudi real-estate corpus with
// 150 labeled probe queries, it beat 3-small by +17.3 pts recall@1 (28.7% →
// 46.0%), +20.7 pts recall@3, +0.19 MRR — at 10ms LOWER query latency.
// Truncated to 1536 dims it stays compatible with the existing column.
// NOTE: after changing this, run scripts/reembed.js — stored vectors and query
...
async function ingestDocument({ companyId, filename, mime, buffer }) {
  ...
  invalidateChunkCache(companyId);   // new chunks must be visible to the keyword leg
...
// ─── Keyword-leg corpus cache ─────────────────────────────────
// The keyword leg needs every chunk text for the company. Fetching all of
// them per query cost ~241KB of transfer at 500 chunks and grew linearly, and
// normalizeArabic() then re-ran over the identical text on every single call.
// Cache the fetched rows WITH their normalized form precomputed, so a warm
// query pays neither the transfer nor the normalization. The corpus only
// changes on KB upload/delete, which call invalidateChunkCache().
const chunkCache = new Map();          // companyId -> { rows, at }
const CHUNK_CACHE_TTL_MS = 5 * 60_000; // backstop for writes we didn't observe

async function getNormalizedChunks(companyId) {
  const hit = chunkCache.get(companyId);
  if (hit && Date.now() - hit.at < CHUNK_CACHE_TTL_MS) return hit.rows;
  const raw = await sql.listCompanyChunkTexts.all(companyId);
  // Sort by id: the query has no ORDER BY, so physical row order varies between
  // calls. The keyword leg assigns RRF ranks by position, so an unstable corpus
  // order silently produced different final rankings for the same question.
  ...
  chunkCache.set(companyId, { rows, at: Date.now() });
  return rows;
}

function invalidateChunkCache(companyId) {
  if (companyId) chunkCache.delete(companyId);
...
async function retrieve(companyId, query, { topK = TOP_K, minScore = MIN_SCORE } = {}) {
  ...
  // The embedding API call (~230ms) and the keyword corpus fetch are fully
  // independent, so run them concurrently instead of queueing the second
  // behind the first. Replaces the old countCompanyChunks round trip too:
  // an empty corpus is simply an empty chunk list.
  const [qVec, corpus] = await Promise.all([
    embedOne(query),
    getNormalizedChunks(companyId),
  ]);
  ...
  // Corpus is already fetched and normalized above.
  const kwRanked = [];
  if (tokens.length) {
    for (const r of corpus) {
      const kw = keywordScore(tokens, r.norm);
      if (kw > 0) kwRanked.push({ id: r.id, documentId: r.documentId, text: r.text, kwScore: kw });
    }
    // id tiebreak: many chunks share an identical keyword score, and their
    // rank position feeds RRF — ties must not be resolved by arrival order.
    kwRanked.sort((a, b) => (b.kwScore - a.kwScore) || (Number(a.id) - Number(b.id)));
  ...
  // Sort by RRF, breaking ties on id. Without the tiebreak, chunks with equal
  // RRF fell back to Map insertion order, which follows the physical row order
  // of an unordered SELECT — so the SAME question returned different chunks
  // between calls (measured: 2 distinct orderings across 6 identical runs).
  // Retrieval must be reproducible, both for callers and for A/B evaluation.
  let results = [...fused.values()]
    .filter((r) => (r.score || 0) >= minScore || (r.kwScore || 0) >= KW_OVERRIDE)
    .sort((a, b) => (b.rrf - a.rrf) || (Number(a.id) - Number(b.id)));
