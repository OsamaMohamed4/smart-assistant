...
// ─── Retrieval ────────────────────────────────────────────────
// Driver-aware: under postgres the search runs in the database via the
// pgvector HNSW index (O(log n)); under sqlite it stays the in-memory
// cosine scan (fine at small scale). Result shape is identical — parity
// verified to 6 decimals by scripts/migrate-to-pg.js.
async function retrieve(companyId, query, { topK = TOP_K, minScore = MIN_SCORE } = {}) {
  ...
  if (isPg) {
    ...
    return retrieveVec(companyId, qVecF32, { topK, minScore });
  }
