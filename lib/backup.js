...
async function runBackup(logger, deps = {}) {
  ...
  // Snapshot format depends on the driver:
  //  - sqlite: db.serialize() → binary .db.gz (open directly with SQLite)
  //  - postgres: JSON dump of every table → .json.gz (restore via script;
  //    Railway PG also keeps its own native backups — this is the offsite copy)
  let snapshot, ext;
  if (data.isPg) {
    const TABLES = ['schema_migrations', 'users', 'companies', 'sessions', 'auth_events',
      'chats', 'calls', 'kb_documents', 'kb_chunks', 'audit_events', 'usage_counters',
      'webhook_events', 'scenarios', 'scenario_versions', 'whatsapp_sessions', 'api_keys'];
    const dump = {};
    for (const t of TABLES) {
      // embeddings/bytea as text — restorable, and keeps JSON serializable
      dump[t] = await data.all(
        t === 'kb_chunks'
          ? `SELECT id, company_id, document_id, chunk_index, text, embedding::text AS embedding, token_count, created_at FROM kb_chunks`
          : `SELECT * FROM ${t}`,
      );
    }
    snapshot = Buffer.from(JSON.stringify(dump));
    ext = 'json.gz';
  } else {
    snapshot = data.db.serialize();                      // consistent, WAL-safe
    ext = 'db.gz';
  }
  const gz = zlib.gzipSync(snapshot);
  const stamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
  const key = `${cfg.prefix}data-${stamp}.${ext}`;
  ...
  try {
    ...
    const keys = (listed.Contents || [])
      .map((o) => o.Key)
      .filter((k) => /data-.*\.(db|json)\.gz$/.test(k))
      .sort()
      .reverse();
